# -*- coding: utf-8 -*-
"""
Created on Thu Jan 18 21:45:40 2024
...
class VAE(AE):
    def __init__(self, in_channels=1, latent_dims=32,beta= 1):
        ...
        self.mu = nn.Sequential(nn.Conv1d(self.encoding_dims,self.latent_dims,kernel_size=3,padding="same"),nn.Tanh())
        self.sigma = nn.Sequential(nn.Conv1d(self.encoding_dims,self.latent_dims,kernel_size=3,padding="same"),nn.Softplus())
        self.flatten=nn.Flatten()
        self.channel_end=64
        self.linear_encode=nn.Conv1d(3*3*self.channel_end,self.encoding_dims,kernel_size=3,padding="same")
        self.decode_layer = nn.Conv1d(self.latent_dims,3*3*self.channel_end,kernel_size=3,padding="same")
        self.label_projector=nn.Conv1d(in_channels=self.num_classes, out_channels=self.latent_dims, kernel_size=3,padding='same')
        
    def encode(self, x):
        x_hidden = self.encoder(x)
        x_hidden=self.flatten(x_hidden)
        x_hidden=x_hidden.unsqueeze(dim=2)
...
def train_VAE(model, dataloader,test_dataloader, epochs=35, lr=1e-4, device = torch.device("cuda"),writer=None, spec_normalizer=lambda x:x,starting_time=""):
...
writer = SummaryWriter(path_dir+f"/runs_conv/model_conv_CVAE_beta{beta}_opti_archi_lattent_{latent_dims}")
...
fig=disp_Mnist_random_sample_condition(model,4,4,Y1)
...
torch.save(model.state_dict(),path_dir+f"/model_save/model_conv_CVAE_beta{beta}_opti_archi_lattent_{latent_dims}")
