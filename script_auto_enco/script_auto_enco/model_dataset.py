...
class ConvVAE(nn.Module):
    def __init__(self):
        ...
        self.enc5=nn.Sequential(nn.Conv2d(16,64,kernel_size=5,stride=2,padding=2),nn.ReLU())#8
        self.enc6=nn.Sequential(nn.Conv2d(64,self.end_size,kernel_size=8,stride=1,padding=0),nn.ReLU())#1
        
        # fully connected layers for learning representations
        self.fc1 = nn.Linear(self.end_size, 256)
        self.fc_mu = nn.Linear(256, 400)
        self.fc_log_var = nn.Linear(256, 400)
        self.fc2 = nn.Linear(400, self.end_size)
        # decoder 
        self.dec1 = nn.Sequential(nn.Upsample(scale_factor=4),
        nn.Conv2d(128,64,kernel_size=5,padding="same"),nn.BatchNorm2d(64),nn.ReLU())
        
        self.dec2 =  nn.Sequential(nn.Upsample(scale_factor=4),
        nn.Conv2d(64,16,kernel_size=5,padding="same"),nn.BatchNorm2d(16),nn.ReLU())
